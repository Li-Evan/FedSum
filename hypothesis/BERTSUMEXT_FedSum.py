...
class Fed_Sum_ExtSummarizer(nn.Module):
    def __init__(self, classifier_type, forzen=False):
        super(Fed_Sum_ExtSummarizer, self).__init__()
    ...
    def only_PLM_forward(self, src, segs, clss, mask_src, mask_cls):
        top_vec = self.bert(src, segs, mask_src)
        sents_vec = top_vec[torch.arange(top_vec.size(0)).unsqueeze(1), clss]
        sents_vec = sents_vec * mask_cls[:, :, None].float()
        return top_vec, sents_vec

    def only_clf_forward(self, sents_vec, mask_cls):
        sent_scores = self.ext_layer(sents_vec, mask_cls).squeeze(-1)
        return sent_scores, mask_cls