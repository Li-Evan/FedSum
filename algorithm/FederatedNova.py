...
# Federated Nova with BERTSUM model
def Fed_Nova_BERTSUMEXT(device,
                       global_model,
    ...
    # FedNova算法中，需要计算一共进行了多少个Batch的更新
    local_update_times_list = [int(math.floor(size/param_dict['batch_size'])) for size in client_datasets_size_list]
    ...
    for iter_t in range(communication_round_I):
        ...
        # FedNova论文Eq.12的“括号”--(  ∑{_i=1}{^m} p_i * τ{_i}{t}  )
        sacling_fator = sum([average_weight[index] * local_update_times_list[index] for index in idxs_users])
                ...
                for batch_index, batch in enumerate(client_i_dataloader):
                    ...
                    try:
                        for i in range(0, len(src), sub_batch_size):
                            ...
                            loss = loss * sacling_fator / local_update_times_list[id]
