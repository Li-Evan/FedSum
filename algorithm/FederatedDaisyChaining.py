...
# Federated Daisy Chaining with BERTSUM model
def Fed_DC_BERTSUMEXT(device,
                       global_model,
    ...
    for iter_t in range(communication_round_I):
        ...
        for index, id in enumerate(idxs_users):
            ...
            # 循环下发到下一个用户
            if index+1 != len(idxs_users):
                next_id = idxs_users[index+1]
                local_model_list[next_id] = copy.deepcopy(model)
